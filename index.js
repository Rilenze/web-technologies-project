...
app.post("/login", async function (req, res) {
  fs.readFile("public/data/nastavnici.json", async function (err, data) {
    ...
    const nastavnik = nastavnici.find(
      (n) => n.nastavnik.username == req.body.username
    );
    ...
    if (!nastavnik) {
      ...
      const isValid = await bcrypt.compare(
        req.body.password,
        nastavnik.nastavnik.password_hash
      );
      if (isValid) {
...
app.get("/predmet/:NAZIV", function (req, res) {
  fs.readFile("public/data/prisustva.json", function (err, data) {
    ...
    const prisustvaZaPredmet = prisustva.find(
      (p) => p.predmet == req.params.NAZIV
    );
...
app.post("/prisustvo/predmet/:NAZIV/student/:index", function (req, res) {
  ...
  fs.readFile("public/data/prisustva.json", function (err, data) {
    ...
    fs.writeFile(
      "public/data/prisustva.json",
      JSON.stringify(prisustvaPredmeta),
      function (err) {
        if (err) console.log(err);
      }
    );

    const predmet = prisustvaPredmeta.find(
      (pr) => pr.predmet == req.params.NAZIV
    );
    res.json(predmet);
  });
});
